...
Unfortunately it ends on five consecutive \texttt{1}'s, meaning that a \texttt{0} must be stuffed in before the delimiter, causing the frame to be one bit longer.\\
...
This will be calculated, as the faster controllers are not available.
 unit time, when excluding the overhead.
...
\begin{equation}
\frac{8}{111 10^-6}= 72072
\end{equation}

That means, that the effective transfer rate is 70.4 kB/s, or 563 kb/s
This is significantly less than stated in the requirements.
\martin{Rewrite, so that we don't mention 8 Mb/s here, move that to the future work. Maybe also mention here, that it give up to 4.6 Mb/s}
