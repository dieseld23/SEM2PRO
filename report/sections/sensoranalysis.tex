...
This section will deal only with parameters the describe the state of the go-kart.
...
\begin{itemize}
	...
	Knowing the currents could also prove to be an invaluable debugging tool when developing a new inverter for the go-kart.\catalin{Can you say that "knowing sth" is a tool?}
...
\begin{itemize}
	...
	\catalin{Mechanical or remote? Or Remote mechanical?}
	\item \textbf{Temperature:} Certain parts of the go-kart directly impact driving performance with changing temperature.
	Additionally, overheating the motor, the batteries or tyres may cause expensive or dangerous failures.\catalin{Tires is with "i", not "y". Tyre is the british spelling}
\end{itemize}
...
\thomas{you can easily leave out that word. If you can save a few words without loosing the meaning, do it.}
...
\thomas{what..? Mechanical means.. well.. Something mechanical.. The physical parameters means all of the parameters that describe the physical state of the go kart as it exists in the world.}
...
\mikkel{If it is of no concern then it should not be in the report? :)}
\mikkel{Add something about the built-in Kalmann filter?}
