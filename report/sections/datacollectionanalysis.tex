...
In addition to this, the it should also be possible to easily add additional data producers at a later time.
...
The single-platform approach would also likely not meet the requirements of the computational requirements set by the equipment.
...
The type of network depends largely on the number of nodes that it has to support.
A project on SDU, Formula Student, has a similar (although slightly more professional) platform for which a network has been developed.
On that platform seven nodes are connected on the network.
The parameters found in section \ref{sec:parameters}
