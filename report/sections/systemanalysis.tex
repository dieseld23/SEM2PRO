...
Starting from the bottom: The CAN bus itself is a differential signal bus, meaning that the absolute value of each wire on the bus doesn't matter, but the difference between them matters. 
...
The CAN transceivers can be bought online, but due to the simplicity of this circuitry, the group will design and manufacture their own.\\
\todo[inline]{Mikkel: Above section is confusing for me.}

The CAN controller can be standalone hardware, but are in many cases built into the micro-controller.
The major advantage with having the controller built into the micro-controller is, that it would not otherwise require another protocol to communicate between the microprocessor and controller. 
...
\todo[inline]{Thomas: Infer new section about protocol, move till after the next section proposed in my todo below.}
