...
It is, that messages are extended by padding.
If either 0 or 1 is written to the controller 5 consecutive times, the opposite polarity is interjected. 
This is done to ensure that receiving nodes remain synchronized with the transmitting node, as there is no clock signal
That means, that a frame can be extended by upwards of 20 \% if 