...
Any data would be could be put into the log, although some signals can be logged at significantly higher rates than others, and might pose a storage problem.
...
The source for the resampled signal is the 1 kSa/s recording.
...
This is determined by the inductance of the motor, which is $600 \si{\micro \henry}$ \todo{when one half bridge is high, and the two others are low, it's one armature inductance of 400 uH in series with two parallel armature inductances}, and the maximum voltage across it, which is $V_{BAT}=52.8 \si{\volt}$. 
